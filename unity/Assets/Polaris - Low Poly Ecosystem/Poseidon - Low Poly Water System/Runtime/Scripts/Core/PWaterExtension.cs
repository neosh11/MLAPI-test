...
namespace Pinwheel.Poseidon
{
    ...
    internal static class PRippleCalculator
    {
        private static Vector3 CalculateRippleVertexOffset(this PWater water, Vector3 localPos, float rippleHeight, float rippleSpeed, float rippleScale, Vector3 flowDir)
        {
            ...
            float noiseBase = PNoiseTextureSampler.SampleVertexNoise((noisePos - flowDirXZ * water.GetTimeParam() * rippleSpeed) * rippleScale) * 0.5f + 0.5f;
            float noiseFade = Mathf.Lerp(0.5f, 1f, PNoiseTextureSampler.SampleVertexNoise((noisePos + flowDirXZ * water.GetTimeParam() * rippleSpeed * 3) * rippleScale) * 0.5f + 0.5f);
